...
const evaluation: EvaluationReport = {
  ...
  provenance: { pluginVersion: "0.1.0", parserVersion: "scanner-v1", graderVersion: "grader-v1", promptVersions: ["none"], policyVersions: ["policy-v1"], schemaVersions: ["finding-v1"], modelProfile: "fixture", fixtureManifestHash: "a".repeat(64), configurationFingerprint: "b".repeat(64), hardware: { platform: "darwin", architecture: "arm64", memoryBytes: 1, runtime: "node" } },
...
describe("release quality report", () => {
  it("fails closed until both provider evidence and manual acceptance are present", () => {
    const report = buildReleaseQualityReport({ generatedAt: "2026-07-29T00:00:00.000Z", evaluation, providerReports: [], manualAcceptance: false });
    expect(report).toMatchObject({ decision: "no-go", privacy: { localByDefault: true, automaticPublishing: false, rawVaultContentIncluded: false } });
