...
selects one, enters an API key, and enables the cloud-data acknowledgement, the
plugin sends the bounded prompt and selected evidence required for that semantic
request only to the provider's fixed API origin. It does not send traces,
