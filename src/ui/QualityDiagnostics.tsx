    ...
    <details className="quality-diagnostics">
      ...
      <section aria-label="Agent performance dashboard">
        ...
        <dl className="observability-configuration">
          <dt>Agent duration</dt><dd>{snapshot.metrics.agentDurationMs} ms</dd>
          <dt>Retries</dt><dd>{snapshot.metrics.retries}</dd>
          <dt>Tokens</dt><dd>{snapshot.metrics.tokenUsage}</dd>
          <dt>Validation failures</dt><dd>{snapshot.metrics.validationFailures}</dd>
          <dt>Open findings</dt><dd>{snapshot.metrics.queueDepth}</dd>
          <dt>Incomplete scan rate</dt><dd>{Math.round(snapshot.metrics.incompleteRate * 100)}%</dd>
        </dl>
      </section>
      <section aria-label="Knowledge health trends">
        ...
        <dl className="observability-configuration">
          <dt>Active / resolved</dt><dd>{health.activeFindings} / {health.resolvedFindings}</dd>
          <dt>Recurring</dt><dd>{health.recurringFindings}</dd>
          <dt>Stale lifecycle records</dt><dd>{health.staleFindings}</dd>
        </dl>
        {health.byType.length > 0 ? (
          <ul className="health-trend-list">
            {health.byType.map((row) => <li key={row.type}>{row.type}: {row.active} active, {row.resolved} resolved, {row.recurring} recurring</li>)}
          </ul>
        ) : null}
      </section>
      <section aria-label="Replay workspace">
        ...
        <p>{scans.length === 0 ? "No eligible fixture replay is retained." : "Fixture source content is unavailable for retained vault scans."}</p>
      </section>
    </details>
  );
}
...
export function AIDebugConsole({ snapshot }: { snapshot: ObservabilitySnapshot }) {
  return (
    <details className="ai-debug-console">
      ...
      <dl className="observability-configuration">
        <dt>Configuration</dt><dd>{snapshot.configuration?.fingerprint ?? "not retained"}</dd>
        <dt>Prompt snapshots</dt><dd>{snapshot.inventory.categories.promptSnapshots.enabled ? "opted in" : "disabled"}</dd>
        <dt>Model-output snapshots</dt><dd>{snapshot.inventory.categories.modelOutputSnapshots.enabled ? "opted in" : "disabled"}</dd>
